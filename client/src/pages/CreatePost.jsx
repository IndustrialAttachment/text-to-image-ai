...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        if (imageCount >= 50) {
          alert('You have reached the limit of 3 images per day. Please contact Mujahidul to generate more.');
          return;
        }

        setGeneratingImg(true);
        const response = await fetch('https://ai-image-ruddy.vercel.app/api/v1/imgGenerate', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://ai-image-ruddy.vercel.app/api/v1/post', {
          method: 'POST',
          headers: {
            // 'Authorization': `Bearer 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`,
            'Content-Type': 'application/json',
  ...
  return (
    <section className="max-w-7xl mx-auto">
      <div className="justify-center text-center">
        ...
        <h1 class="mb-4 text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl dark:text-black">Create <mark class="px-2 text-white bg-red-600 rounded dark:bg-red-500">AI</mark> Images</h1>
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-5">
          <FormField
            ...
            type="text"
            name="name"
            placeholder="Ex. Mujahidul Islam"
            value={form.name}
